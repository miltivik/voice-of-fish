...
pub fn download_model_file(models_path: &Path, model_id: &str, app: Option<&tauri::AppHandle>) -> Result<Vec<LocalModel>, String> {
    let entry = model_catalog()
        .into_iter()
        .find(|m| m.id == model_id)
        .ok_or_else(|| format!("unknown model id: {model_id}"))?;

    let url = entry.download_url.as_deref().filter(|u| !u.is_empty())
...
pub fn verify_checksum(path: &Path, expected: &str) -> Result<(), String> {
    ...
    if computed == expected.to_lowercase() {
        ...
        Err(format!("checksum mismatch: expected {e}, got {c}", e = expected.to_lowercase(), c = computed))
    }
}
...
mod tests {
    ...
    fn list_detects_installed() {
        ...
        let m = list_local_models(dir.path()).into_iter().find(|m| m.id == "s2-q6").unwrap();
    ...
    fn delete_removes_file() {
        ...
        assert_eq!(models.iter().find(|m| m.id == "s2-q6").unwrap().state, ModelState::NotInstalled);
        assert!(!fp.exists());
    }

    #[test]
    fn delete_unknown_errors() {
        assert!(delete_model_file(TempDir::new().unwrap().path(), "nope").unwrap_err().contains("unknown"));
    }
    ...
    fn download_no_url_fails() {
        assert!(download_model_file(TempDir::new().unwrap().path(), "nonexistent-model", None).unwrap_err().contains("unknown model id"));
    }
    ...
    fn checksum_ok() {
        ...
        f.flush().unwrap();
        assert!(verify_checksum(f.path(), "b94d27b9934d3e08a52e52d7da7dabfac484efe37a5380ee9088f7ace2efcde9").is_ok());
    }
    ...
    fn checksum_mismatch() {
        ...
        f.write_all(b"hello world").unwrap();
        f.flush().unwrap();
        assert!(verify_checksum(f.path(), &"0".repeat(64)).unwrap_err().contains("mismatch"));
    }
