...
export function EngineHeader() {
  ...
  const activeModel = S2_MODEL_MANIFEST.find((model) => model.id === activeModelId);
  ...
  return (
    <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-line bg-studio/95 px-6 backdrop-blur">
      <div>
        <p className="text-xs font-medium uppercase tracking-wide text-muted">Engine status</p>
      ...
      <div className="flex items-center gap-3">
        ...
        <Badge>
          ...
          {activeModel ? `${activeModel.id.toUpperCase()} ${activeModel.quant}` : activeModelId}
        </Badge>
      </div>
    </header>
  );
